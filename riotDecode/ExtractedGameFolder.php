...
class ExtractedGameFolder {
	...
	public function getRiotArchiveFiles() {
		if($this->riotArchiveFiles === null) {
			...
			foreach(scandir($this->path . '\\') as $versionFolder) {
				if(preg_match("/([0-9]+\.[0-9]+\.[0-9]+)/", $versionFolder, $matches)) {
					...
					//	if(preg_match("/\\.raf$/", $potentialRiotArchiveFile)) {
							$this->riotArchiveFiles[$matches[1]] = new \riotDecode\raf\ExtractedRiotArchiveFile($this->path . '\\' . $matches[1]);
					//	}
					//}
				}
			}
